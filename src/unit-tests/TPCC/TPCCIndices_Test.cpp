...
void TPCCIndices_Test::setup() {
	...
	size_t orderTableSize = clientsCnt * ORDER_BUFFER_PER_CLIENT;
	size_t orderLineTableSize = clientsCnt * tpcc_settings::ORDER_MAX_OL_CNT * ORDER_BUFFER_PER_CLIENT;
	size_t newOrderTableSize = clientsCnt * ORDER_BUFFER_PER_CLIENT;
	...
	context = new RDMAContext(std::cout, config::IB_PORT[0]);
...
void TPCCIndices_Test::test_clearOrderIndex(){
	...
	try{
		db->orderTable.registerOrderInIndex(wID, dID, cID, oID, clientID, regionOffset);
...
void TPCCIndices_Test::test_getExistingOrderAddressByOrderID(){
	...
	try{
		...
		db->orderTable.getOrderMemoryAddress(wID, dID, oID, &returnedClientWhoOrdered, &returnedRegionOffset);
...
void TPCCIndices_Test::test_getLastOrderOfCustomer(){
	...
	try{
		db->orderTable.registerOrderInIndex(wID, dID, cID, oID_1, clientID_1, regionOffset_1);
		db->orderTable.registerOrderInIndex(wID, dID, cID, oID_2, clientID_2, regionOffset_2);
