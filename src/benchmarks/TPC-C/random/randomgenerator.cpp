...
std::set<int> RandomGenerator::selectUniqueIds(size_t num_unique, int lower_id, int upper_id) {
	std::set<int> rows;
...
std::set<uint32_t> RandomGenerator::selectUniqueNonUniformIds(int A, size_t num_unique, int lower_id, int upper_id){
	...
	for (size_t i = 0; i < num_unique; ++i) {
		int index = -1;
		do {
			index = NURand(A, lower_id, upper_id);
		} while (rows.find(index) != rows.end());
		rows.insert((uint32_t)index);
	}
	assert(rows.size() == num_unique);
...
void makeLastName(int num, char* name) {
	...
	int offset = 0;
	for (unsigned int i = 0; i < sizeof(indicies)/sizeof(*indicies); ++i) {
		ASSERT(strlen(SYLLABLES[indicies[i]]) == (size_t)LENGTHS[indicies[i]]);
		std::memcpy(name + offset, SYLLABLES[indicies[i]], LENGTHS[indicies[i]]);
...
RealRandomGenerator::RealRandomGenerator() {
	...
	int result = initstate_r(static_cast<unsigned int>(utils::generate_random_seed()), state_array,
