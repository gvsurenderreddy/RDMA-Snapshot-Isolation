...
int TimestampServerContext::register_memory() {
	...
	TEST_Z(mr_read_timestamp	= ibv_reg_mr(pd, &read_timestamp, sizeof(Timestamp), mr_flags));
	TEST_Z(mr_commit_timestamp	= ibv_reg_mr(pd, &commit_timestamp, sizeof(Timestamp), mr_flags));
	return 0;
}
...
int TimestampServerContext::destroy_context () {
	...
	if (mr_read_timestamp)	TEST_NZ (ibv_dereg_mr (mr_read_timestamp));
