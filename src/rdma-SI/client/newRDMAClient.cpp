...
void RDMAClient::fill_shopping_cart() {
	...
	for (int i=0; i < config::ORDERLINE_PER_ORDER; i++) {
		...
		ds_ctx[i].associated_cart_line = &cart.cart_lines[i];
	}
}
...
int RDMAClient::acquire_read_ts() {
	...
	ts_ctx.qp,
	ts_ctx.read_timestamp_mr,
	(uint64_t)&ts_ctx.read_timestamp,
	&(ts_ctx.peer_mr_read_timestamp),
	(uint64_t)ts_ctx.peer_mr_read_timestamp.addr,
	(uint32_t)sizeof(Timestamp),
	true));
	TEST_NZ (RDMACommon::poll_completion(ts_ctx.send_cq));
	return 0;
}

int RDMAClient::get_item_info(const size_t server_num) {
	int item_id = ds_ctx[server_num].associated_cart_line->SCL_I_ID;
	...
	ItemVersion *item_lookup_address =  (ItemVersion *)(item_offset + ((uint64_t)ds_ctx[server_num].peer_mr_items.addr));

	// RDMA READ it from the server
	TEST_NZ( RDMACommon::post_RDMA_READ_WRT(IBV_WR_RDMA_READ,
	ds_ctx[server_num].qp,
	ds_ctx[server_num].items_mr,
	(uint64_t)ds_ctx[server_num].items_region,
	&(ds_ctx[server_num].peer_mr_items),
...
int RDMAClient::acquire_commit_ts() {
	...
	ts_ctx.qp,
	ts_ctx.commit_timestamp_mr,
	(uint64_t)&ts_ctx.commit_timestamp,
	&(ts_ctx.peer_mr_commit_timestamp),
	(uint64_t)ts_ctx.peer_mr_commit_timestamp.addr,
	(uint64_t)1ULL,
	(uint32_t)sizeof(Timestamp)));
	TEST_NZ (RDMACommon::poll_completion(ts_ctx.send_cq));
	
	// Byte swapping, since values read by atomic operations are in Big Endian order
	ts_ctx.commit_timestamp.value = bigEndianToHost(ts_ctx.commit_timestamp.value);
	return 0;
}
...
int RDMAClient::acquire_item_lock(const size_t server_num, uint64_t *expected_lock, uint64_t *new_lock) {
	...
	memset(ds_ctx[server_num].lock_items_region, 0, sizeof(uint64_t));
	
	size_t lock_offset			= item_id * sizeof(uint64_t);
	uint64_t *lock_lookup_addr	= (uint64_t *)(lock_offset + ((uint64_t)ds_ctx[server_num].peer_mr_lock_items.addr));

	TEST_NZ (RDMACommon::post_RDMA_CMP_SWAP(ds_ctx[server_num].qp,
	ds_ctx[server_num].lock_items_mr,
	(uint64_t)ds_ctx[server_num].lock_items_region,
	&(ds_ctx[server_num].peer_mr_lock_items),
...
int RDMAClient::revert_lock(const size_t server_num, uint64_t *new_lock) {
	...
	memcpy(ds_ctx[server_num].lock_items_region, new_lock, sizeof(uint64_t));
...
int RDMAClient::update_item_stock(const size_t server_num) {
	int item_id		= ds_ctx[server_num].associated_cart_line->SCL_I_ID;
	int quantity	= ds_ctx[server_num].associated_cart_line->SCL_QTY;
	
	int fetch_block_num = 0;
	
	ds_ctx[server_num].items_region[0].write_timestamp.value = ts_ctx.commit_timestamp.value;
	ds_ctx[server_num].items_region[0].item.I_STOCK 	-= quantity;
	
	if (ds_ctx[server_num].items_region[0].item.I_STOCK < 10)
		ds_ctx[server_num].items_region[0].item.I_STOCK += 20;
	...
	ItemVersion *item_lookup_addr =  (ItemVersion *)(item_offset + ((uint64_t)ds_ctx[server_num].peer_mr_items.addr));
	...
	DEBUG_COUT (CLASS_NAME, __func__, "[WRIT] ... Successfully updated the stock for item " << item_id
		<< " (with commit timestamp " << ts_ctx.commit_timestamp.value << ")");
...
int RDMAClient::release_lock(const size_t server_num) {
	...
	version			= (uint32_t) ts_ctx.commit_timestamp.value;
	//version			= (uint32_t) 0;
	
	ds_ctx[server_num].lock_items_region[0] = Lock::set_lock(lock_status, version);
...
int RDMAClient::submit_trx_result(Result result, int transactionNum) {
	if (transactionNum > 1)
		TEST_NZ (RDMACommon::poll_completion(ts_ctx.recv_cq));	// collecting the ack for the previous request

	bool signaled = false;
	if (transactionNum % 5000 == 0)
		signaled = true;

	ts_ctx.transaction_result.ts.value = ts_ctx.commit_timestamp.value;
	ts_ctx.transaction_result.result = result;

	TEST_NZ (RDMACommon::post_RECEIVE(ts_ctx.qp, ts_ctx.transaction_result_ack_mr, (uintptr_t)&ts_ctx.transaction_result_ack, sizeof(struct message::TransactionResult)));
	TEST_NZ (RDMACommon::post_SEND(ts_ctx.qp, ts_ctx.transaction_result_mr, (uintptr_t)&ts_ctx.transaction_result, sizeof(struct message::TransactionResult), signaled));

	if (signaled)
		TEST_NZ (RDMACommon::poll_completion(ts_ctx.send_cq));

	DEBUG_COUT (CLASS_NAME, __func__, "[Sent] Transaction result for CTS " << ts_ctx.transaction_result.ts.value << " sent to the TS server");
	return 0;
}
...
int RDMAClient::startTransactions() {
	...
	for (int trx_num = 1; trx_num <= config::TRANSACTION_CNT; trx_num++){
		...
		DEBUG_COUT (CLASS_NAME, __func__, "[Read] Step 1: Received read timestamp from oracle with TID " << ts_ctx.read_timestamp.value);
		...
		for (int i = 0; i < config::ORDERLINE_PER_ORDER; i++) {
			...
			server_num = cart.cart_lines[i].SCL_I_ID / config::ITEM_PER_SERVER;
			TEST_NZ(get_item_info(server_num));
		}
		for (int i = 0; i < config::ORDERLINE_PER_ORDER; i++) {
			server_num = cart.cart_lines[i].SCL_I_ID / config::ITEM_PER_SERVER;
			TEST_NZ (RDMACommon::poll_completion(ds_ctx[server_num].send_cq));
			DEBUG_COUT (CLASS_NAME, __func__, "[Read] Step 2-" << i << ": Received information for item " << ds_ctx[server_num].items_region->item.I_ID
					<< "(ts: " << ds_ctx[server_num].items_region->write_timestamp.value << ") from " << ds_ctx[server_num].server_address);
		}
		...
		if (true) {
			...
			// ************************************************	
			acquire_commit_ts();
			DEBUG_COUT (CLASS_NAME, __func__, "[FTCH] Step 3: Acquired commit timestamp " << ts_ctx.commit_timestamp.value);
			...
			for (int i = 0; i < config::ORDERLINE_PER_ORDER; i++) {
				...
				lock_status					= (uint32_t) 0;
				version						= (uint32_t) ds_ctx[server_num].items_region[found_version_index].write_timestamp.value;
			...
			for (int i = 0; i < config::ORDERLINE_PER_ORDER; i++) {
				...
				// Byte swapping, since values read by atomic operations are in Big Endian order
				ds_ctx[server_num].lock_items_region[0] = bigEndianToHost(ds_ctx[server_num].lock_items_region[0]);

				// Check if CASing the lock was successful
				if (Lock::are_equals(ds_ctx[server_num].lock_items_region[0], expected_lock[server_num])) {
					...
					DEBUG_COUT (CLASS_NAME, __func__, "........ Existing lock: " << Lock::get_lock_status(ds_ctx[server_num].lock_items_region[0])
						<< " | " << Lock::get_version(ds_ctx[server_num].lock_items_region[0]));
			...
			if (abort_flag == true) {
				...
				for (int i = 0; i < config::ORDERLINE_PER_ORDER; i++) {
					...
					if (successful_locking_servers[server_num] == true) {
						TEST_NZ (RDMACommon::poll_completion(ds_ctx[server_num].send_cq));
						DEBUG_COUT (CLASS_NAME, __func__, "[Writ] Step 4: Lock reverted on item " << ds_ctx[server_num].items_region->item.I_ID);
					}
				}
				...
				for (int i = 0; i < config::ORDERLINE_PER_ORDER; i++) {
					...
					DEBUG_COUT (CLASS_NAME, __func__, "[Writ] Lock on item " << cart.cart_lines[i].SCL_I_ID
						<< " released with commit timestamp " << ts_ctx.commit_timestamp.value);
				}
...
int RDMAClient::start_client () {	
	 ...
	 ************************************************/
	ts_ctx.server_address = "";
	ts_ctx.server_address += config::TIMESTAMP_SERVER_ADDR;
	ts_ctx.ib_port	= config::TIMESTAMP_SERVER_IB_PORT;
	TEST_NZ (establish_tcp_connection(ts_ctx.server_address, config::TIMESTAMP_SERVER_PORT, &ts_ctx.sockfd));
	DEBUG_COUT(CLASS_NAME, __func__, "[Conn] Connection established to Timestamp Server");
	TEST_NZ (ts_ctx.create_context ());
	...
	RDMACommon::post_RECEIVE(ts_ctx.qp, ts_ctx.recv_mr, (uintptr_t)&ts_ctx.recv_msg, sizeof(struct message::TimestampServerMemoryKeys));

	TEST_NZ (RDMACommon::connect_qp (&(ts_ctx.qp), ts_ctx.ib_port, ts_ctx.port_attr.lid, ts_ctx.sockfd));
	DEBUG_COUT(CLASS_NAME, __func__, "[Conn] QPed to Timestamp Server");


	TEST_NZ(RDMACommon::poll_completion(ts_ctx.recv_cq));
	...
	memcpy(&ts_ctx.peer_mr_read_timestamp,	&ts_ctx.recv_msg.mr_read_timestamp,		sizeof(struct ibv_mr));
	memcpy(&ts_ctx.peer_mr_commit_timestamp,&ts_ctx.recv_msg.mr_commit_timestamp,	sizeof(struct ibv_mr));
	...
	for (int i = 0; i < config::SERVER_CNT; i++){
		ds_ctx[i].server_address = "";
		ds_ctx[i].server_address += config::SERVER_ADDR[i];
		ds_ctx[i].ib_port		  = config::IB_PORT[i];
		
		// Connect to servers
		TEST_NZ (establish_tcp_connection(ds_ctx[i].server_address, config::TCP_PORT[i], &ds_ctx[i].sockfd));
		DEBUG_COUT(CLASS_NAME, __func__, "[Conn] Connection established to server " << i);
	
		TEST_NZ (ds_ctx[i].create_context());
		...
		RDMACommon::post_RECEIVE(ds_ctx[i].qp, ds_ctx[i].recv_mr, (uintptr_t)&ds_ctx[i].recv_msg, sizeof(struct message::DataServerMemoryKeys));
	
		TEST_NZ (RDMACommon::connect_qp (&(ds_ctx[i].qp), ds_ctx[i].ib_port, ds_ctx[i].port_attr.lid, ds_ctx[i].sockfd));
		DEBUG_COUT(CLASS_NAME, __func__, "[Conn] QPed to server " << i);
	
		TEST_NZ(RDMACommon::poll_completion(ds_ctx[i].recv_cq));
		DEBUG_COUT(CLASS_NAME, __func__, "[Recv] buffers info from server " << i);
	
		// after receiving the message from the server, let's store its addresses in the context
		memcpy(&ds_ctx[i].peer_mr_items,		&ds_ctx[i].recv_msg.mr_items,		sizeof(struct ibv_mr));
		memcpy(&ds_ctx[i].peer_mr_orders,		&ds_ctx[i].recv_msg.mr_orders,		sizeof(struct ibv_mr));
		memcpy(&ds_ctx[i].peer_mr_order_line,	&ds_ctx[i].recv_msg.mr_order_line,	sizeof(struct ibv_mr));
		memcpy(&ds_ctx[i].peer_mr_cc_xacts,		&ds_ctx[i].recv_msg.mr_cc_xacts,	sizeof(struct ibv_mr));
		memcpy(&ds_ctx[i].peer_mr_timestamp,	&ds_ctx[i].recv_msg.mr_timestamp,	sizeof(struct ibv_mr));
		memcpy(&ds_ctx[i].peer_mr_lock_items,	&ds_ctx[i].recv_msg.mr_lock_items,	sizeof(struct ibv_mr));
	}
	...
	for (int i = 0; i < config::SERVER_CNT; i++) {
		TEST_NZ (sock_sync_data (ds_ctx[i].sockfd, 1, "W", &temp_char));	/* just send a dummy char back and forth */
		DEBUG_COUT(CLASS_NAME, __func__, "[Conn] Notified server " << i << " it's done");
		TEST_NZ ( ds_ctx[i].destroy_context());
	}
	TEST_NZ (sock_sync_data (ts_ctx.sockfd, 1, "W", &temp_char));	/* just send a dummy char back and forth */
