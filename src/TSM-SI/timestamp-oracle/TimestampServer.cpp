...
int TimestampServer::register_memory(struct WorkerContext &ctx) {
	...
	TEST_Z(ctx.mr_last_trx_timestamp_per_client= ibv_reg_mr(ctx.pd, last_trx_timestamp_per_client_,	sizeof(primitive::timestamp_t) * clients_cnt_, atomicFlag));
	return 0;
}
...
void TimestampServer::handle_client(struct WorkerContext &ctx, primitive::client_id_t client_id) {
	...
	memcpy(&(ctx.send_msg.mr_last_trx_timestamp_per_client),	ctx.mr_last_trx_timestamp_per_client,	sizeof(struct ibv_mr));
...
int TimestampServer::destroy_client_context (struct WorkerContext &ctx) {
	...
	if (ctx.mr_last_trx_timestamp_per_client)
		TEST_NZ (ibv_dereg_mr (ctx.mr_last_trx_timestamp_per_client));
...
TimestampServer::TimestampServer(size_t clients_cnt)
: clients_cnt_ (clients_cnt),
  server_sockfd_ (-1) {

	// initialize the finished_trxs_hash
	last_trx_timestamp_per_client_ = new primitive::timestamp_t[clients_cnt_];
	...
	for (size_t i = 0; i < clients_cnt_; i++)
		last_trx_timestamp_per_client_[i] = 0;
